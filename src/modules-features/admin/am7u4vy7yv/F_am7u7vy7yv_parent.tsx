...
import I_am7u4vy7yv_Children from "./F_am7u4vy7yv_Children";
...
interface I_am7u4vy7yv_parent {
  ...
  children: I_am7u4vy7yv_parent[];
  created_at: string;
  updated_at: string;
}

export default function I_am7u4vy7yv_modal({ slug }: { slug: string }) {
  const disc = useDisclosure(false);

  const { data: children, isLoading } = useQuery<I_am7u4vy7yv_parent[]>({
  ...
  const columns = useMemo<MRT_ColumnDef<I_am7u4vy7yv_parent>[]>(() => [
    ...
    },
    {
          header: "Children",
          Cell: ({ row }) => {
            return <I_am7u4vy7yv_Children slug={row.original.slug} />;
          },
        },
  ...
  return (
    <MyButtonModal crudType="check" disclosure={disc} modalSize={"80%"}>
      <Text fw={600} size="lg">Danh sách danh mục của danh mục `{slug}`</Text>
      <MyDataTable
        enableRowSelection
        enableRowNumbers
        exportAble
